...
import { DeliveryVersusPaymentV1, IDeliveryVersusPaymentV1 } from '@typechain/contracts/dvp/V1/DeliveryVersusPaymentV1';
...
describe(`DeliveryVersusPaymentV1 Network:${network.name}`, async () => {
  before(async function () {
    ...
    // Deploy DVP
    const dvpTx = await new DeliveryVersusPaymentV1__factory(_deployer.wallet).deploy();
    ...
    // Malicious token
    const maliciousTokenFactory = new MaliciousTokenDVP__factory(_deployer.wallet);
    _maliciousToken = await maliciousTokenFactory.deploy('MaliciousToken', 'MAL', _dvp);
    await _maliciousToken.waitForDeployment();
    _maliciousTokenAddress = await _maliciousToken.getAddress();

    // Malicious actor
    const maliciousActorFactory = new MaliciousActorDVP__factory(_deployer.wallet);
    ...
    const revertTokenFactory = new AssetTokenThatReverts__factory(_deployer.wallet);
    ...
    await _usdcToken.connect(_deployer.wallet).mint(_alice.address, TOKEN_AMOUNT_LARGE_6_DECIMALS);
    await _usdcToken.connect(_deployer.wallet).mint(_bob.address, TOKEN_AMOUNT_LARGE_6_DECIMALS);
    await _usdcToken.connect(_deployer.wallet).mint(_charlie.address, TOKEN_AMOUNT_LARGE_6_DECIMALS);

    await _usdtToken.connect(_deployer.wallet).mint(_alice.address, TOKEN_AMOUNT_LARGE_6_DECIMALS);
    await _usdtToken.connect(_deployer.wallet).mint(_bob.address, TOKEN_AMOUNT_LARGE_6_DECIMALS);
    await _usdtToken.connect(_deployer.wallet).mint(_charlie.address, TOKEN_AMOUNT_LARGE_6_DECIMALS);

    await _daiToken.connect(_deployer.wallet).mint(_alice.address, TOKEN_AMOUNT_LARGE_18_DECIMALS);
    await _daiToken.connect(_deployer.wallet).mint(_bob.address, TOKEN_AMOUNT_LARGE_18_DECIMALS);
    await _daiToken.connect(_deployer.wallet).mint(_charlie.address, TOKEN_AMOUNT_LARGE_18_DECIMALS);

    await _maliciousToken.connect(_deployer.wallet).mint(_alice.address, TOKEN_AMOUNT_LARGE_18_DECIMALS);
    await _maliciousToken.connect(_deployer.wallet).mint(_bob.address, TOKEN_AMOUNT_LARGE_18_DECIMALS);
    await _maliciousToken.connect(_deployer.wallet).mint(_charlie.address, TOKEN_AMOUNT_LARGE_18_DECIMALS);

    // Mint NFTs - Alice has two cats
    await _nftCatToken.connect(_deployer.wallet).mint(_alice.address, NFT_CAT_DAISY);
    await _nftCatToken.connect(_deployer.wallet).mint(_alice.address, NFT_CAT_BUTTONS);

    // Mint NFTs - Charlie has two dogs
    await _nftDogToken.connect(_deployer.wallet).mint(_charlie.address, NFT_DOG_FIDO);
    await _nftDogToken.connect(_deployer.wallet).mint(_charlie.address, NFT_DOG_TOBY);
    ...
    _sanctionsList = await new SanctionsList__factory(_deployer.wallet).deploy(_deployer.address);
  ...
  describe('[Function] createSettlement', async () => {
    it('[Case 1] Should succeed with valid flows and future cutoff date', async () => {
      ...
      const tx = await _dvp.connect(_alice.wallet).createSettlement(flows, SETTLEMENT_REF, cutoff, false);
    ...
    it('[Case 2] Should succeed with empty reference field', async () => {
      const flows = buildFlows(_fb);
      const cutoff = _timestamps.nowPlusDays(7);
      const tx = await _dvp.connect(_alice.wallet).createSettlement(flows, '', cutoff, false);
      const receipt = await tx.wait();
      const createSettlementEvent = findEvent(receipt, 'SettlementCreated');
      expect(createSettlementEvent).not.to.be.undefined;
    ...
    it('[Case 4] Should revert if no flows provided', async () => {
      const flows: Flow[] = [];
      await expect(
        _dvp.connect(_alice.wallet).createSettlement(flows, SETTLEMENT_REF, _timestamps.nowPlusDays(7), false)
      ).to.be.revertedWithCustomError(_dvp, CUSTOM_ERRORS.DeliveryVersusPayment.NoFlowsProvided);
    });

    it('[Case 5] Should revert if cutoff date is in the past', async () => {
      const flows = buildFlows(_fb);
      await expect(
        _dvp.connect(_alice.wallet).createSettlement(flows, SETTLEMENT_REF, _timestamps.nowMinusDays(1), false)
      ).to.be.revertedWithCustomError(_dvp, CUSTOM_ERRORS.DeliveryVersusPayment.CutoffDatePassed);
    });
    ...
    it('[Case 6] Should revert if supplied NFT token is really an ERC-20', async () => {
      ...
      await expect(
        _dvp.connect(_alice.wallet).createSettlement(flows, SETTLEMENT_REF, _timestamps.nowPlusDays(7), false)
      ).to.be.revertedWithCustomError(_dvp, CUSTOM_ERRORS.DeliveryVersusPayment.InvalidERC721Token);
    });
    ...
    it('[Case 9] Should revert if supplied ERC-20 token is really an NFT', async () => {
      ...
      await expect(
        _dvp.connect(_alice.wallet).createSettlement(flows, SETTLEMENT_REF, _timestamps.nowPlusDays(7), false)
      ).to.be.revertedWithCustomError(_dvp, CUSTOM_ERRORS.DeliveryVersusPayment.InvalidERC20Token);
    });
  ...
  describe('[Function] getSettlement', async () => {
    it('[Case 1] Should succeed', async () => {
      ...
      const [storedRef, storedCutoff, storedFlows, isSettled, isAutoSettled] = await _dvp.getSettlement(1n);
    ...
    it('[Case 3] Should revert if settlement does not exist', async () => {
      await expect(_dvp.getSettlement(NOT_A_SETTLEMENT_ID)).to.be.revertedWithCustomError(
        _dvp,
  ...
  describe('[Function] approveSettlements', async () => {
    ...
    it('[Case 1] Should succeed for a single settlement requiring ETH deposit', async () => {
      ...
      const tx = await _dvp.connect(_bob.wallet).approveSettlements(settleIds, { value: totalEthRequired });
      const receipt = await tx.wait();
      ...
      await expect(_dvp.isSettlementApproved(NOT_A_SETTLEMENT_ID)).to.be.revertedWithCustomError(
        _dvp,
        CUSTOM_ERRORS.DeliveryVersusPayment.SettlementDoesNotExist
      );

      // Check deposit recorded
      const { etherDeposited } = await _dvp.getSettlementPartyStatus(settlementId, _bob.address);
    ...
    it('[Case 2] Should revert if settlement already executed', async () => {
      ...
      await _usdcToken.connect(_alice.wallet).approve(_dvpAddress, TOKEN_AMOUNT_SMALL_6_DECIMALS);
      await _daiToken.connect(_charlie.wallet).approve(_dvpAddress, TOKEN_AMOUNT_SMALL_18_DECIMALS);

      await _dvp.connect(_alice.wallet).executeSettlement(settlementId);

      await expect(_dvp.connect(_alice.wallet).approveSettlements([settlementId])).to.be.revertedWithCustomError(
        _dvp,
    ...
    it('[Case 5] Should revert if caller not involved in the settlement', async () => {
      ...
      await expect(_dvp.connect(_dave.wallet).approveSettlements([settlementId])).to.be.revertedWithCustomError(
        _dvp,
    ...
    it('[Case 6] Should revert if incorrect ETH amount sent', async () => {
      ...
      await expect(
        _dvp.connect(_bob.wallet).approveSettlements([settlementId], { value: requiredEth - 1n })
      ).to.be.revertedWithCustomError(_dvp, CUSTOM_ERRORS.DeliveryVersusPayment.IncorrectETHAmount);
    ...
    it('[Case 7] Should succeed approving multiple settlements at once', async () => {
      ...
      const tx2 = await _dvp.connect(_alice.wallet).createSettlement(flows2, SETTLEMENT_REF, cutoff2, false);
      const receipt2 = await tx2.wait();
      const settlementId2 = findEvent(receipt2, 'SettlementCreated').args.settlementId;

      // For settlementId2 no ETH is required from Bob
      await _dvp.connect(_bob.wallet).approveSettlements([settlementId, settlementId2], {
        value: TOKEN_AMOUNT_SMALL_18_DECIMALS
      });

      // Check approvals
      const { etherDeposited: deposit1 } = await _dvp.getSettlementPartyStatus(settlementId, _bob.address);
      expect(deposit1).to.equal(TOKEN_AMOUNT_SMALL_18_DECIMALS);

      const { etherDeposited: deposit2 } = await _dvp.getSettlementPartyStatus(settlementId2, _bob.address);
      expect(deposit2).to.equal(0n);
    });

    it('[Case 8] Should revert if settlement does not exist', async () => {
      await expect(_dvp.connect(_alice.wallet).approveSettlements([NOT_A_SETTLEMENT_ID])).to.be.revertedWithCustomError(
        _dvp,
        CUSTOM_ERRORS.DeliveryVersusPayment.SettlementDoesNotExist
      );
    });
    ...
    it('[Case 9] Should succeed for a single settlement with auto settlement', async () => {
      ...
      const cutoff = _timestamps.nowPlusDays(7);
      const tx = await _dvp.connect(_alice.wallet).createSettlement(flows, SETTLEMENT_REF, cutoff, true);
      const receipt = await tx.wait();
      const createSettlementEvent = findEvent(receipt, 'SettlementCreated');
      expect(createSettlementEvent).not.to.be.undefined;
      const { settlementId } = createSettlementEvent.args;
      ...
      await _usdcToken.connect(_alice.wallet).approve(_dvpAddress, TOKEN_AMOUNT_SMALL_6_DECIMALS);
      const tx2 = await _dvp.connect(_alice.wallet).approveSettlements([settlementId]);
      const receipt2 = await tx2.wait();
    ...
    it('[Case 10] Should succeed for multiple settlements mixing regular and auto settlement', async () => {
      ...
      const cutoff = _timestamps.nowPlusDays(7);
      const txRegular = await _dvp.connect(_alice.wallet).createSettlement(flowsRegular, SETTLEMENT_REF, cutoff, false);
      const receiptRegular = await txRegular.wait();
      const createSettlementEventRegular = findEvent(receiptRegular, 'SettlementCreated');
      expect(createSettlementEventRegular).not.to.be.undefined;
      const { settlementId: settlementIdRegular } = createSettlementEventRegular.args;
      const { isAutoSettled: isAutoSettledRegular } = await _dvp.getSettlement(settlementIdRegular);
      ...
      const txAuto = await _dvp.connect(_alice.wallet).createSettlement(flowsAuto, SETTLEMENT_REF, cutoff, true);
      const receiptAuto = await txAuto.wait();
      const createSettlementEventAuto = findEvent(receiptAuto, 'SettlementCreated');
      expect(createSettlementEventAuto).not.to.be.undefined;
      const { settlementId: settlementIdAuto } = createSettlementEventAuto.args;
      const { isAutoSettled: isAutoSettledAuto } = await _dvp.getSettlement(settlementIdAuto);
      expect(isAutoSettledAuto).to.be.true;

      // Alice will approve both settlements, but only the auto-settlement should be executed
      await _usdcToken.connect(_alice.wallet).approve(_dvpAddress, TOKEN_AMOUNT_SMALL_6_DECIMALS * 2n);
      const txApprove = await _dvp.connect(_alice.wallet).approveSettlements([settlementIdRegular, settlementIdAuto]);
      ...
      expect(executedEvent.args.executor).to.equal(_alice.address);
      const { isSettled: isSettledRegular } = await _dvp.getSettlement(settlementIdRegular);
      expect(isSettledRegular).to.be.false;
      const { isSettled: isSettledAuto } = await _dvp.getSettlement(settlementIdAuto);
    ...
    it('[Case 11] Should succeed for approval with NFT', async () => {
      ...
      await _nftCatToken.connect(_alice.wallet).approve(_dvpAddress, NFT_CAT_DAISY);
      const txApprove = await _dvp.connect(_alice.wallet).approveSettlements([settlementId]);
      const receiptApprove = await txApprove.wait();
    ...
    it('[Case 12] Should succeed for a single settlement with auto settlement but execution reverts with safeTransferFrom default message', async () => {
      ...
      // If Alice now approves, since she's the only approver, the settlement should auto-execute
      await _revertToken.connect(_alice.wallet).approve(_dvpAddress, TOKEN_AMOUNT_SMALL_6_DECIMALS);
      const tx2 = await _dvp.connect(_alice.wallet).approveSettlements([settlementId]);
      const receipt2 = await tx2.wait();
      // So should see settlement approved
      const approvedEvent = findEvent(receipt2, 'SettlementApproved');
      ...
      // but not executed
      const executionFailedEvent = findEvent(receipt2, 'SettlementAutoExecutionFailedOther');
    ...
    it('[Case 13] Should succeed for a single settlement with auto settlement but execution reverts with reason string', async () => {
      ...
      // but not executed
      const executionFailedEvent = findEvent(receipt2, 'SettlementAutoExecutionFailedReason');
      expect(executionFailedEvent).not.to.be.undefined;
      expect(executionFailedEvent.args.settlementId).to.equal(settlementId);
      expect(executionFailedEvent.args.executor).to.equal(_alice.address);
    ...
    it('[Case 14] Should succeed for a single settlement with auto settlement but execution reverts with panic', async () => {
      ...
      // but not executed
      const executionFailedEvent = findEvent(receipt2, 'SettlementAutoExecutionFailedPanic');
      expect(executionFailedEvent).not.to.be.undefined;
      expect(executionFailedEvent.args.settlementId).to.equal(settlementId);
      expect(executionFailedEvent.args.executor).to.equal(_alice.address);
    ...
    it('[Case 16] Should succeed for multiple settlements all with auto settlement but some execution reverts', async () => {
      ...
      const cutoff = _timestamps.nowPlusDays(7);
      let tx = await _dvp.connect(_alice.wallet).createSettlement(flowsExeOk, SETTLEMENT_REF, cutoff, true);
      ...
      tx = await _dvp.connect(_alice.wallet).createSettlement(flowsExeRevert, SETTLEMENT_REF, cutoff, true);
      receipt = await tx.wait();
      createSettlementEvent = findEvent(receipt, 'SettlementCreated');
      expect(createSettlementEvent).not.to.be.undefined;
      const { settlementId: settlementIdExeRevert } = createSettlementEvent.args;

      // Alice will approve both settlements, but only the auto-settlement should be executed
      await _usdcToken.connect(_alice.wallet).approve(_dvpAddress, TOKEN_AMOUNT_SMALL_6_DECIMALS * 2n);
      await _revertToken.connect(_alice.wallet).approve(_dvpAddress, TOKEN_AMOUNT_SMALL_6_DECIMALS * 2n);
      ...
      const { isSettled: isSettledExeOk } = await _dvp.getSettlement(settlementIdExeOk);
      expect(isSettledExeOk).to.be.true;

      // one settlement approved but doesnt execute
      const executionFailedEvent = findEvent(receiptApprove, 'SettlementAutoExecutionFailedOther');
      expect(executionFailedEvent).not.to.be.undefined;
      expect(executionFailedEvent.args.settlementId).to.equal(settlementIdExeRevert);
      expect(executionFailedEvent.args.executor).to.equal(_alice.address);
      expect(executionFailedEvent.args.lowLevelData).to.equal('0x');
      expect(await _dvp.isSettlementApproved(settlementIdExeOk)).to.be.true;
      const { isSettled: isSettledExeRevert } = await _dvp.getSettlement(settlementIdExeRevert);
  ...
  describe.only('[Function] executeSettlement', async () => {
    ...
    beforeEach(async () => {
      ...
      const txMixed = await _dvp.connect(_alice.wallet).createSettlement(flowsMixed, SETTLEMENT_REF, cutoff, false);
      const receiptMixed = await txMixed.wait();
      settlementMixedId = findEvent(receiptMixed, 'SettlementCreated').args.settlementId;
      ...
      await _dvp.connect(_charlie.wallet).approveSettlements([settlementMixedId]);

      // Approve that DVP contract can spend tokens & NFTs
      await _usdcToken.connect(_charlie.wallet).approve(_dvpAddress, TOKEN_AMOUNT_SMALL_6_DECIMALS);
      await _nftCatToken.connect(_alice.wallet).approve(_dvpAddress, NFT_CAT_DAISY);
    });
    ...
    it('[Case 1] Should succeed if all approved and conditions met', async () => {
      ...
      const oldCharlieEthBalance = await ethers.provider.getBalance(_charlie.address);

      // Execute settlement
      const tx = await _dvp.connect(_alice.wallet).executeSettlement(settlementId);
      const receipt = await tx.wait();
      ...
      const newCharlieEthBalance = await ethers.provider.getBalance(_charlie.address);
      expect(newCharlieEthBalance - oldCharlieEthBalance).to.equal(TOKEN_AMOUNT_SMALL_18_DECIMALS);

      // Bob's deposit should now be zero since it has been transferred to Charlie
      const { etherDeposited: bobDepositAfter } = await _dvp.getSettlementPartyStatus(settlementId, _bob.address);
      ...
      expect(aliceDaiBalanceAfter - oldAliceDaiBalance).to.equal(TOKEN_AMOUNT_SMALL_18_DECIMALS);
    ...
    it('[Case 2] Should succeed for mixed flows with NFTs if all approved and conditions met', async () => {
      ...
      const bobIsCatOwnerBefore = _bob.address == (await _nftCatToken.ownerOf(NFT_CAT_DAISY));
      const charlieEthBalanceBefore = await ethers.provider.getBalance(_charlie.address);
      expect(bobIsCatOwnerBefore).to.be.false;

      // Execute settlement
      const tx = await _dvp.connect(_alice.wallet).executeSettlement(settlementMixedId);
      const receipt = await tx.wait();
      const executedEvent = findEvent(receipt, 'SettlementExecuted');
      expect(executedEvent).not.to.be.undefined;
      ...
      const bobIsCatOwnerAfter = _bob.address == (await _nftCatToken.ownerOf(NFT_CAT_DAISY));
      expect(bobIsCatOwnerAfter).to.be.true;

      // Check ETH transfer (Bob -> Charlie)
      const charlieEthBalanceAfter = await ethers.provider.getBalance(_charlie.address);
      expect(charlieEthBalanceAfter - charlieEthBalanceBefore).to.equal(TOKEN_AMOUNT_SMALL_18_DECIMALS);

      // Check token transfer (Charlie -> Alice) USDC
      const aliceUsdcBalanceAfter = await _usdcToken.balanceOf(_alice.address);
      expect(aliceUsdcBalanceAfter - aliceUsdcBalanceBefore).to.equal(TOKEN_AMOUNT_SMALL_6_DECIMALS);
    ...
    it('[Case 3] Should succeed for with NFT approved with setApprovalForAll', async () => {
      ...
      await _nftCatToken.connect(_alice.wallet).setApprovalForAll(_dvpAddress, true);
      const bobIsCatOwnerBefore = _bob.address == (await _nftCatToken.ownerOf(NFT_CAT_DAISY));
      expect(bobIsCatOwnerBefore).to.be.false;
    ...
    it('[Case 6] Should revert if not all approved', async () => {
      ...
      await expect(_dvp.connect(_alice.wallet).executeSettlement(settlementId2)).to.be.revertedWithCustomError(
        _dvp,
    ...
    it('[Case 7] Should revert if insufficient ETH deposited', async () => {
      ...
      await expect(_dvp.connect(_bob.wallet).approveSettlements([settlementId2])).to.be.revertedWithCustomError(
        _dvp,
    ...
    it('[Case 10] Should revert on attempted re-entrancy by token', async () => {
      ...
      const cutoff = _timestamps.nowPlusDays(7);
      const createTx = await _dvp.connect(_alice.wallet).createSettlement(flows, SETTLEMENT_REF, cutoff, false);
      const createReceipt = await createTx.wait();
      const settlementId = findEvent(createReceipt, 'SettlementCreated').args.settlementId;

      // Alice approves settlement
      await _dvp.connect(_alice.wallet).approveSettlements([settlementId]);

      // Attempt to execute the settlement causes maliciousToken.transferFrom() to trigger re-entrancy
      _maliciousToken.connect(_alice.wallet).setTargetSettlementId(settlementId);
      await expect(_dvp.connect(_alice.wallet).executeSettlement(settlementId)).to.be.revertedWithCustomError(
        _dvp,
    ...
    it('[Case 11] Should revert on attempted re-entrancy by ETH wallet', async () => {
      ...
      // Alice approves settlement
      await _dvp.connect(_alice.wallet).approveSettlements([settlementId], { value: 1n });

      // Attempt to execute the settlement causes _maliciousActor.receive() to trigger re-entrancy
      _maliciousActor.connect(_deployer.wallet).setTargetSettlementId(settlementId);
      _maliciousActor.connect(_deployer.wallet).setReentrancyMode(ReentrancyMode.ExecuteSettlement);
    ...
    it('[Case 12] Should revert if settlement does not exist', async () => {
      await expect(_dvp.connect(_alice.wallet).executeSettlement(NOT_A_SETTLEMENT_ID)).to.be.revertedWithCustomError(
        _dvp,
        CUSTOM_ERRORS.DeliveryVersusPayment.SettlementDoesNotExist
      );
    });
  ...
  describe('[Function] revokeApprovals', async () => {
    ...
    it('[Case 1] Should succeed when returning ETH deposit', async () => {
      ...
      const { etherDeposited: bobDepositBefore } = await _dvp.getSettlementPartyStatus(settlementId, _bob.address);
      expect(bobDepositBefore).to.equal(TOKEN_AMOUNT_SMALL_18_DECIMALS);

      // Revoke approval
      const tx = await _dvp.connect(_bob.wallet).revokeApprovals([settlementId]);
      const receipt = await tx.wait();
    ...
    it('[Case 2] Should succeed when no ETH deposit', async () => {
      ...
      const { etherDeposited: aliceDepositBefore } = await _dvp.getSettlementPartyStatus(settlementId2, _alice.address);
      expect(aliceDepositBefore).to.equal(0n);
      const revokeTx = await _dvp.connect(_alice.wallet).revokeApprovals([settlementId2]);
      ...
      const { etherDeposited: aliceDepositAfter } = await _dvp.getSettlementPartyStatus(settlementId2, _alice.address);
    ...
    it('[Case 3] Should revert if settlement already executed', async () => {
      ...
      await expect(_dvp.connect(_alice.wallet).revokeApprovals([settlementId])).to.be.revertedWithCustomError(
        _dvp,
        CUSTOM_ERRORS.DeliveryVersusPayment.SettlementAlreadyExecuted
      );
    });
    ...
    it('[Case 4] Should revert if approval not granted by caller', async () => {
      ...
      const cutoff2 = _timestamps.nowPlusDays(7);
      const tx2 = await _dvp.connect(_charlie.wallet).createSettlement(flows2, SETTLEMENT_REF, cutoff2, false);
      const receipt2 = await tx2.wait();
      const settlementId2 = findEvent(receipt2, 'SettlementCreated').args.settlementId;

      // Charlie did not approve yet
      await expect(_dvp.connect(_alice.wallet).revokeApprovals([settlementId2])).to.be.revertedWithCustomError(
        _dvp,
    ...
    it('[Case 5] Should revert on attempted re-entrancy by ETH wallet', async () => {
      ...
      const createTx2 = await _dvp.connect(_alice.wallet).createSettlement(flows2, SETTLEMENT_REF, cutoff2, false);
      const createReceipt2 = await createTx2.wait();
      const settlementId2 = findEvent(createReceipt2, 'SettlementCreated').args.settlementId;

      // Malicious actor sets up target + re-entrancy mode = RevokeApproval
      await _maliciousActor.connect(_deployer.wallet).setTargetSettlementId(settlementId2);
      await _maliciousActor.connect(_deployer.wallet).setReentrancyMode(ReentrancyMode.RevokeApproval);

      // Approve settlement with 1 wei deposit from malicious actor
      await _maliciousActor.connect(_deployer.wallet).approveSettlement({ value: 1n });

      // Attempt to revoke ETH approval ultimately causes _maliciousActor.receive() to trigger re-entrancy
      await expect(_maliciousActor.connect(_deployer.wallet).revokeApproval()).to.be.revertedWithCustomError(
        _dvp,
        CUSTOM_ERRORS.DeliveryVersusPayment.ReentrancyGuardReentrantCall
      );
    });
    ...
    it('[Case 6] Should revert if settlement does not exist', async () => {
      await expect(_dvp.connect(_alice.wallet).revokeApprovals([NOT_A_SETTLEMENT_ID])).to.be.revertedWithCustomError(
        _dvp,
        CUSTOM_ERRORS.DeliveryVersusPayment.SettlementDoesNotExist
      );
    });
  ...
  describe('[Function] withdrawETH', async () => {
    ...
    it('[Case 1] Should succeed with withdrawal of ETH deposit after cutoff', async () => {
      ...
      expect(withdrawEvent.args.amount).to.equal(TOKEN_AMOUNT_SMALL_18_DECIMALS);
    ...
    it('[Case 2] Should revert if cutoff date not passed', async () => {
      await expect(_dvp.connect(_bob.wallet).withdrawETH(settlementId)).to.be.revertedWithCustomError(
        _dvp,
        CUSTOM_ERRORS.DeliveryVersusPayment.CutoffDatePassed
      );
    });
    ...
    it('[Case 4] Should revert if no ETH to withdraw', async () => {
      ...
      await expect(_dvp.connect(_alice.wallet).withdrawETH(settlementId2)).to.be.revertedWithCustomError(
        _dvp,
    ...
    it('[Case 5] Should revert on attempted re-entrancy by ETH wallet', async () => {
      ...
      await _maliciousActor.connect(_deployer.wallet).setTargetSettlementId(settlementId);
      await _maliciousActor.connect(_deployer.wallet).setReentrancyMode(ReentrancyMode.WithdrawETH);
      await _maliciousActor.connect(_deployer.wallet).approveSettlement({ value: 1n });
      ...
      await expect(_maliciousActor.connect(_deployer.wallet).withdrawETH()).to.be.revertedWithCustomError(
        _dvp,
        CUSTOM_ERRORS.DeliveryVersusPayment.ReentrancyGuardReentrantCall
      );
    });
    ...
    it('[Case 6] Should revert if settlement does not exist', async () => {
      await expect(_dvp.connect(_bob.wallet).withdrawETH(NOT_A_SETTLEMENT_ID)).to.be.revertedWithCustomError(
        _dvp,
        CUSTOM_ERRORS.DeliveryVersusPayment.SettlementDoesNotExist
      );
    });
  ...
  describe('[Function] getSettlementPartyStatus', () => {
    it('[Case 1] Should show correct statuses for parties throughout settlement lifecycle', async () => {
      ...
      const tx = await _dvp.connect(_alice.wallet).createSettlement(flows, SETTLEMENT_REF, cutoff, false);
      const receipt = await tx.wait();
      const settlementId = findEvent(receipt, 'SettlementCreated').args.settlementId;
      ...
      // Approvals
      //--------------------------------------------------------------------------------
      await _dvp.connect(_alice.wallet).approveSettlements([settlementId], { value: 1_000_000n });
      await _dvp.connect(_bob.wallet).approveSettlements([settlementId], { value: 1_500_000n });
      await _dvp.connect(_charlie.wallet).approveSettlements([settlementId]);
    ...
    it('[Case 2] Should show correct statuses for parties throughout settlement lifecycle with NFTs', async () => {
      ...
      await _nftCatToken.connect(_alice.wallet).approve(_dvpAddress, NFT_CAT_DAISY);
      await _nftCatToken.connect(_alice.wallet).approve(_dvpAddress, NFT_CAT_BUTTONS);

      // Bob approves his USDC tokens.
      await _usdcToken.connect(_bob.wallet).approve(_dvpAddress, TOKEN_AMOUNT_SMALL_6_DECIMALS);

      // Charlie approves his NFT from _nftDog.
      await _nftDogToken.connect(_charlie.wallet).approve(_dvpAddress, NFT_DOG_FIDO);
        ...
        tokenStatuses: [
          {
            ...
            amountOrIdRequired: TOKEN_AMOUNT_SMALL_6_DECIMALS,
            amountOrIdApprovedForDvp: 0n,
            amountOrIdHeldByParty: TOKEN_AMOUNT_LARGE_6_DECIMALS - TOKEN_AMOUNT_SMALL_6_DECIMALS
          }
        ]
      });
    ...
    it('[Case 3] Should revert if settlement does not exist', async () => {
      await expect(_dvp.getSettlementPartyStatus(NOT_A_SETTLEMENT_ID, _alice.address)).to.be.revertedWithCustomError(
        _dvp,
        CUSTOM_ERRORS.DeliveryVersusPayment.SettlementDoesNotExist
      );
    });
    ...
    it('[Case 4] Should return empty results if party not involved in settlement', async () => {
      ...
      const receipt = await tx.wait();
      const settlementId = findEvent(receipt, 'SettlementCreated').args.settlementId;
      const partyStatus = await _dvp.getSettlementPartyStatus(settlementId, _dave.address);
    ...
    it('[Case 5] Should achieve full coverage for party status', async () => {
      ...
      const receipt = await tx.wait();
      const settlementId = findEvent(receipt, 'SettlementCreated').args.settlementId;
      const partyStatus = await _dvp.getSettlementPartyStatus(settlementId, _alice.address);
      expect(partyStatus.isApproved).to.be.false;
    });
  });
  ...
  describe('[Process] Send ETH directly to contract', async () => {
    it('[Case 1] Should revert if Ether is sent directly to the contract', async () => {
      ...
      ).to.be.revertedWithCustomError(_dvp, CUSTOM_ERRORS.DeliveryVersusPayment.CannotSendEtherDirectly);
    });
  });
