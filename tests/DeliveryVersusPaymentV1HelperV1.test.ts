...
describe(`DeliveryVersusPaymentV1HelperV1 on network:${network.name}`, function () {
  before(async function () {
    ...
    _dvp = await new DeliveryVersusPaymentV1__factory(_deployer.wallet).deploy();
    _dvpAddress = await _dvp.getAddress();
    await _dvp.waitForDeployment();

    // Deploy helper contract pointing to the DVP contract
    _dvpHelper = await new DeliveryVersusPaymentV1HelperV1__factory(_deployer.wallet).deploy();
    ...
    for (let i = 0; i < settlementsToCreatePerType; i++) {
      // Mixed flows (contain multiple token types)
      {
        ...
        const tx = await _dvp.connect(_alice.wallet).createSettlement(flows, `Type mixed ${i}`, cutoff, false);
      ...
      // NFT-only flows
      {
        ...
        const tx = await _dvp.connect(_alice.wallet).createSettlement(flows, `Type NFT ${i}`, cutoff, false);
        const receipt = await tx.wait();
        const event = findEvent(receipt, 'SettlementCreated');
        expect(event).to.exist;
      }
      // ERC20-only flows
      {
        ...
        const tx = await _dvp.connect(_alice.wallet).createSettlement(flows, `Type ERC20 ${i}`, cutoff, false);
        const receipt = await tx.wait();
        const event = findEvent(receipt, 'SettlementCreated');
        expect(event).to.exist;
      }
      // Ether-only flows
      {
        ...
        const tx = await _dvp.connect(_alice.wallet).createSettlement(flows, `Type Ether ${i}`, cutoff, false);
        const receipt = await tx.wait();
        const event = findEvent(receipt, 'SettlementCreated');
        expect(event).to.exist;
      }
  ...
  describe('[Function] getSettlementsByToken()', function () {
    it('[Case 1] Should succeed returning settlements that include the given token (USDC)', async function () {
      const pageSize = 5;
      const { settlementIds } = await _dvpHelper.getSettlementsByToken(_dvpAddress, _usdc, 0, pageSize);
    ...
    it('[Case 2] Should succeed returning an empty array when no settlements match the token', async function () {
      ...
      const pageSize = 5;
      const { settlementIds, nextCursor } = await _dvpHelper.getSettlementsByToken(
        _dvpAddress,
        randomToken,
        0,
        pageSize
    ...
    it('[Case 3] Should revert for an invalid pageSize', async function () {
      await expect(_dvpHelper.getSettlementsByToken(_dvpAddress, _usdc, 0, 1)).to.be.revertedWithCustomError(
        _dvpHelper,
        CUSTOM_ERRORS.DeliveryVersusPaymentHelper.InvalidPageSize
      );
      await expect(_dvpHelper.getSettlementsByToken(_dvpAddress, _usdc, 0, 201)).to.be.revertedWithCustomError(
        _dvpHelper,
        CUSTOM_ERRORS.DeliveryVersusPaymentHelper.InvalidPageSize
      );
    });
  ...
  describe('[Function] getSettlementsByInvolvedParty()', function () {
    it('[Case 1] Should succeed returning settlements involving the given party', async function () {
      const pageSize = 5;
      const { settlementIds } = await _dvpHelper.getSettlementsByInvolvedParty(_dvpAddress, _bob.address, 0, pageSize);
      expect(settlementIds.length).to.be.greaterThan(0);
      for (const id of settlementIds) {
        const settlement = await _dvp.getSettlement(id);
        const involved = settlement.flows.some(flow => flow.from === _bob.address || flow.to === _bob.address);
    ...
    it('[Case 2] Should succeed returning an empty array when the party is not involved in any settlement', async function () {
      ...
      const pageSize = 5;
      const { settlementIds } = await _dvpHelper.getSettlementsByInvolvedParty(_dvpAddress, randomParty, 0, pageSize);
      expect(settlementIds.length).to.equal(0);
    });

    it('[Case 3] Should revert for an invalid pageSize', async function () {
      await expect(
        _dvpHelper.getSettlementsByInvolvedParty(_dvpAddress, _bob.address, 0, 1)
      ).to.be.revertedWithCustomError(_dvpHelper, CUSTOM_ERRORS.DeliveryVersusPaymentHelper.InvalidPageSize);
      await expect(
        _dvpHelper.getSettlementsByInvolvedParty(_dvpAddress, _bob.address, 0, 201)
      ).to.be.revertedWithCustomError(_dvpHelper, CUSTOM_ERRORS.DeliveryVersusPaymentHelper.InvalidPageSize);
    });
  });
  ...
  describe('[Function] getSettlementsByTokenType()', function () {
    it('[Case 1] Should succeed returning settlements for token type Ether', async function () {
      const pageSize = 5;
      const { settlementIds } = await _dvpHelper.getSettlementsByTokenType(_dvpAddress, TokenType.Ether, 0, pageSize);
      expect(settlementIds.length).to.be.greaterThan(0);
      for (const id of settlementIds) {
        const settlement = await _dvp.getSettlement(id);
        // Ether flows have token equal to _eth (i.e. ZeroAddress)
        const hasEtherFlow = settlement.flows.some(flow => flow.token === _eth);
    ...
    it('[Case 2] Should succeed returning settlements for token type ERC20', async function () {
      const pageSize = 5;
      const { settlementIds } = await _dvpHelper.getSettlementsByTokenType(_dvpAddress, TokenType.ERC20, 0, pageSize);
      expect(settlementIds.length).to.be.greaterThan(0);
      for (const id of settlementIds) {
        const settlement = await _dvp.getSettlement(id);
        // ERC20 flows have a non-zero token and isNFT === false
        const hasERC20Flow = settlement.flows.some(flow => flow.token !== _eth && flow.isNFT === false);
    ...
    it('[Case 3] Should succeed returning settlements for token type NFT', async function () {
      const pageSize = 5;
      const { settlementIds } = await _dvpHelper.getSettlementsByTokenType(_dvpAddress, TokenType.NFT, 0, pageSize);
      expect(settlementIds.length).to.be.greaterThan(0);
      for (const id of settlementIds) {
        const settlement = await _dvp.getSettlement(id);
        // NFT flows have a non-zero token and isNFT === true
        const hasNFTFlow = settlement.flows.some(flow => flow.token !== _eth && flow.isNFT === true);
    ...
    it('[Case 4] Should revert for an invalid pageSize', async function () {
      ...
      await expect(
        _dvpHelper.getSettlementsByTokenType(_dvpAddress, TokenType.Ether, 0, 201)
      ).to.be.revertedWithCustomError(_dvpHelper, CUSTOM_ERRORS.DeliveryVersusPaymentHelper.InvalidPageSize);
    });
    ...
    it('[Case 5] Should correctly use a non-zero startCursor for pagination (ERC20)', async function () {
      ...
      const { settlementIds, nextCursor } = await _dvpHelper.getSettlementsByTokenType(
        _dvpAddress,
        TokenType.ERC20,
        0,
        pageSize
      ...
      if (nextCursor !== 0n) {
        const { settlementIds: nextPageIds } = await _dvpHelper.getSettlementsByTokenType(
          _dvpAddress,
          TokenType.ERC20,
          nextCursor,
          pageSize
  ...
  describe('[Process] Pagination behavior', function () {
    ...
    it('[Case 2] Should succeed with paginate results for getSettlementsByInvolvedParty', async function () {
      ...
      let cursor = 0n;
      do {
        const { settlementIds, nextCursor } = await _dvpHelper.getSettlementsByInvolvedParty(
          _dvpAddress,
          _alice.address,
          cursor,
          pageSize
